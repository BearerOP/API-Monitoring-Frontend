...
import {
  ...
  DialogDescription,
  DialogFooter,
} from '@/Components/ui/dialog'; // Ensure correct import paths
...
const ViewAllApi = () => {
    ...
    if (token && selectedLogId) {
      try {
        ...
        if (response && response.data && response.data.success) {
          ...
          setApiLogs((prevLogs) =>
            prevLogs.filter((log) => log._id !== selectedLogId)
          );
        } else {
  ...
  const renderApiLogs = () => {
    return apiLogs.map((log) => {
      ...
      const timeDifference = formatDistanceToNow(latestTimestamp, {
        addSuffix: true,
      });
      return (
        <TableRow className="flex items-center justify-between" key={log._id}>
          <TableCell className="flex items-center w-2/5">
            <span
              className={`mx-3 ${lastLog.status === 'Up' ? 'blinker-up' : 'blinker-down'}`}
            ></span>
            <div>
              <div className="flex gap-1.5 ml-4 align-bottom">
                <Link
                  to={`/dashboard/monitor/viewApi/${log._id}`}
                  className=" hover:cursor-pointer hover:underline  hover:underline-offset-4 place-self-center"
                >
                  {extractDomain(log.url)}
                </Link>
                <Link
                  to={`/dashboard/monitor/viewApi/${log._id}`}
                  className="flex align-center justify-center buttonArrow"
                >
          ...
          <TableCell className="flex gap-x-2 items-center justify-center ">
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>Checked every 5 minutes</TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </TableCell>
          <TableCell>
        ...
        <div>
            ...
            <DialogContent className="dark">
              <DialogHeader>
                ...
                <DialogDescription>
                  ...
                  log and remove it from our servers.
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>
                <Button
          ...
          <Card className="mb-4" style={{ padding: 0 }}>
            <CardContent
              ...
              style={{ maxHeight: '535px', padding: 0 }}
            >
              <Table style={{ padding: 0 }}>
                <TableBody style={{ padding: 0 }}>{renderApiLogs()}</TableBody>
              </Table>
            </CardContent>
          </Card>
        </div>
